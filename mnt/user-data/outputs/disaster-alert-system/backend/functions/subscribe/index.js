...
// Simple phone validation (E.164 format: +1234567890)
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // 2. Subscribe email to SNS topic (for direct SNS delivery as backup)
    if (email && SNS_TOPIC_ARN) {
      try {
        await sns.send(new SubscribeCommand({
          ...
          Protocol: "email",
          Endpoint: email,
          // Filter policy: only send alerts at or above their threshold
          Attributes: {
        ...
        // Don't fail the whole request if SNS subscribe fails
        console.warn("SNS subscribe failed:", snsErr.message);
      }
    }

    // 3. Subscribe phone to SNS topic for SMS
